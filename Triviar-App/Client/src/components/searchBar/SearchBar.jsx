...
const SearchBar = ({setSearch, search, setResSearch}) => {
  ...
  const getProfileSearch = async (event) => {
    try {
      ...
      const { data } = await axios(`${URL_BASE}/profile/search?userName=${input}`);
  ...
  return(
    <>
      <form className={`w-10/12 sm:w-96 h-24 bg-purple-500 absolute  flex items-center justify-center gap-2 rounded-1 z-30 px-2 border-1 border-purple-600 shadow-md shadow-purple-400
      ...
      <div 
      className={`${search ? "w-full h-screen fixed top-0 bg-zinc-500 z-20 opacity-80" : "none"}`}
      onClick={()=>setSearch(!search)}
      >
      </div>
    </>

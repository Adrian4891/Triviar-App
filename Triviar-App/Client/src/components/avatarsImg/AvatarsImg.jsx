    ...
    return(
        <>
            ...
            : "d-none"}`}>
                ...
                <span 
                className="absolute top-0 right-0 m-2 rounded-full text-xl font-bold text-red-500 hover:text-red-400"
                ...
                { avatars.map((avatar, index)=>{
                    return(
                        ...
                        onClick={()=>setOpenList(!openList, setUserData({...userData, ["picture"]:avatar.webformatURL}))}
                        />
                    )})
                }
            </div>
            <div 
            className={`${openList ? "bg-zinc-200 z-10 w-full h-screen fixed opacity-70" : "none" }`}
            onClick={()=>setOpenList(!openList)}
            >
            </div>
        </>

...
const TablesGame = () => {
    ...
    const checkTable = async (id) => {
        try{
           const { data } = await axios.put(`${URL_BASE}/tables?userId=${id}&userIdInvitation=${userId}`);
    ...
    return(
        <section className="w-full min-h-screen bg-blue-100 flex justify-start items-center flex-col gap-5">
            ...
            <div className="flex justify-center items-center flex-wrap gap-5 pt-5">
                {tables.length < 1 && 
                  <h1 className="font-semibold text-xl text-gray-400" >Aún no hay mesas creadas.</h1>}
                {
                    tables.map((table)=>{
                        return(
                            <div 
                            ...
                            className="bg-white border rounded-2 w-96 h-24 shadow-md shadow-gray-300 flex justify-center items-center flex-col gap-2 relative hover:ring-2 hover:ring-blue-400 hover:shadow-lg hover:shadow-gray-400">
                                ...
                                >{`Mesa creada por ${table.userName}`}</p>
                               { userId === table.userId && 
                               <button 
                                ...
                                >X</button>}
                               { userId !== table.userId &&
                                <button className="bg-gray-700 hover:bg-gray-900 font-semibold rounded-1 text-white py-1 px-3 w-24 shadow-md shadow-gray-400 hover:shadow-lg hover:shadow-gray-500"
            ...
            <button className="bg-blue-500 text-white py-1 px-5 rounded-1 font-semibold hover:bg-blue-400 hover:shadow-lg hover:shadow-gray-400"
            onClick={postTable}
            >Crear Mesa</button>
        </section>
    );

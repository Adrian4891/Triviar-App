...
const QuestionAndOptions = (
    {index, answers, handleWinPoints, getAnswers, infoGame, checked, setInfoGame, setIndex, setLoad }
    ) => { 
               ...
               <>
                    ...
                    { answers[index]?.type === "multiple" ?
                        <div className="grid grid-cols-1 md:gap-3 md:grid-cols-2 ">
                            <fieldset className={`px-3 py-1 rounded mb-2 sm:mb-0 mr-1 w-72 lg:w-96 cursor-pointer ${checked === false && "bg-purple-700"}
                            ${(answers[index]?.options[0] === answers[index]?.correct_answer && checked ) && "bg-green-500"} 
                            ${(answers[index]?.options[0] !== answers[index]?.correct_answer && checked)&& "bg-red-500"}`}
                            onClick={()=>activeInp && handleWinPoints(answers[index]?.options[0])}
                            >
                            ...
                            <fieldset className={`px-3 py-1 rounded mb-2 sm:mb-0 mr-1 w-72 lg:w-96 cursor-pointer ${checked === false && "bg-fuchsia-600"}
                            ${(answers[index]?.options[1] === answers[index]?.correct_answer && checked) && "bg-green-500"} 
                            ${(answers[index]?.options[1] !== answers[index]?.correct_answer && checked)&& "bg-red-500"}`}
                            onClick={()=>activeInp && handleWinPoints(answers[index]?.options[1])}
                            >
                            ...
                            <fieldset className={`px-3 py-1 rounded mb-2 sm:mb-0 mr-1 w-72 lg:w-96 cursor-pointer ${checked === false && "bg-indigo-600"}
                            ${(answers[index]?.options[2] === answers[index]?.correct_answer && checked ) && "bg-green-500" }  
                            ${(answers[index]?.options[2] !== answers[index]?.correct_answer && checked)&& "bg-red-500"} `}
                            onClick={()=>activeInp && handleWinPoints(answers[index]?.options[2])}
                            >
                            ...
                            <fieldset className={`px-3 py-1 rounded mb-2 mr-1 w-72 lg:w-96 cursor-pointer ${checked === false && "bg-yellow-500"}                    
                            ${(answers[index]?.options[3] === answers[index]?.correct_answer && checked) && "bg-green-500"} 
                            ${(answers[index]?.options[3] !== answers[index]?.correct_answer && checked)&& "bg-red-500"} `}
                            onClick={()=>activeInp && handleWinPoints(answers[index]?.options[3])}
                            >
                                <label htmlFor="" className="text-white ">D</label>
                                <input
                                type="button" 
                        ...
                        :
                        <div>
                            <fieldset className={`px-3 py-1 rounded mb-2 w-72 lg:w-96 cursor-pointer ${ checked === false && "bg-cyan-600"}
                            ${("True" === answers[index]?.correct_answer && checked) && "bg-green-500"} 
                            ${("True" !== answers[index]?.correct_answer && checked)&& "bg-red-500"}`}
                            ...
                            <fieldset className={`px-3 py-1  rounded mb-2 w-72 lg:w-96 cursor-pointer ${ checked === false && "bg-cyan-600"}
                            ${("False" === answers[index]?.correct_answer && checked) && "bg-green-500"} 
                            ${("False" !== answers[index]?.correct_answer && checked)&& "bg-red-500"} `}
                    ...
                    {!activeInp && <p className="text-white">{players.length > 1  && `Esperado al otro jugador...`}</p>}
                </>
                :
                <div className="grid grid-cols-1 gap-3 sm:gap-5 text-center p-2 w-full content-center justify-items-center	">
                    ...
                    <div className="grid grid-cols-2  content-center justify-items-center">
                        ...
                        className="font-bold text-white text-center border-1 m-2 border-green-500 rounded-1 text-lg hover:bg-green-500 w-32 sm:w-60"
                            onClick={()=> players.length > 1 ?
                                getAnswers(newGame(),socket.emit("name", name), setActiveInp(false))
                                :
